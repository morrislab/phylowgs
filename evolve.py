...
def start_new_run(state_manager, backup_manager, safe_to_exit, run_succeeded, config, ssm_file, cnv_file,
                  top_k_trees_file, clonal_freqs_file, burnin_samples, num_samples, mh_itr, mh_std, write_state_every,
                  write_backups_every, rand_seed, tmp_dir):
    state = {}
    ...
    state['tssb'] = TSSB(dp_alpha=state['dp_alpha'], root_node=root, data=codes)
    root.tssb = state['tssb'] # shankar/phylowgs/treecrp change
    # hack...
    if 0:
        depth = 0
        state['tssb'].root['sticks'] = vstack(
            [state['tssb'].root['sticks'], boundbeta(1, state['tssb'].dp_gamma) if depth != 0 else .999])
        state['tssb'].root['children'].append({'node': state['tssb'].root['node'].spawn(),
                                               'main': boundbeta(1.0,
                                                                 (state['tssb'].alpha_decay ** (depth + 1)) * state[
                                                                     'tssb'].dp_alpha) if state['tssb'].min_depth <= (
                                               depth + 1) else 0.0,
                                               'sticks': empty((0, 1)),
                                               'children': []})
    ...
    do_mcmc(state_manager, backup_manager, safe_to_exit, run_succeeded, config, state, tree_writer, codes, n_ssms,
            n_cnvs, NTPS, tmp_dir)
...
def resume_existing_run(state_manager, backup_manager, safe_to_exit, run_succeeded, config):
    ...
    do_mcmc(state_manager, backup_manager, safe_to_exit, run_succeeded, config, state, tree_writer, codes, n_ssms,
            n_cnvs, NTPS, state['tmp_dir'])


def do_mcmc(state_manager, backup_manager, safe_to_exit, run_succeeded, config, state, tree_writer, codes, n_ssms,
            n_cnvs, NTPS, tmp_dir_parent, pc_sm=True,ls_sm=True):
    ...
    map_datum_to_node(state['tssb']) # phylowgs/treecrp change, shankar

    parent_child_sm_counter = 0
    leaf_sibling_sm_counter = 0
    for iteration in range(start_iter, state['num_samples']):
        ...
        if pc_sm:
            #Split-merge
            old_llh = tssb.complete_log_likelihood()
            tssb2 = deepcopy(tssb)
            hasting_ratio = tssb2.child_parent_split_merge()
            new_llh = tssb2.complete_log_likelihood()
            a = new_llh - old_llh + log(hasting_ratio)
            if log(rand()) < a:
                tssb = tssb2
                if iter > 0: parent_child_sm_counter += 1
        ...
        if ls_sm:
            old_llh = tssb.complete_log_likelihood()
            tssb2 = deepcopy(tssb)
            hasting_ratio = tssb2.leaf_sibling_split_merge()
            new_llh = tssb2.complete_log_likelihood()
            a = new_llh - old_llh + log(hasting_ratio)
            if log(rand()) < a:
                tssb = tssb2
                if iter > 0: leaf_sibling_sm_counter += 1
        ...
        if iteration >= 0:
            ...
            if True or mod(iteration, 10) == 0:
                ...
                logmsg(' '.join([str(v) for v in (
                iteration, len(nodes), state['cd_llh_traces'][iteration], state['mh_acc']
                )]))
        ...
        if should_write_backup or should_write_state or is_last_iteration:
            with open('mcmc_samples.txt', 'a') as mcmcf:
                llhs_and_times = [(itr, llh, itr_time) for (tssb, itr, llh), itr_time in
                                  zip(unwritten_trees, mcmc_sample_times)]
                llhs_and_times = '\n'.join(
                    ['%s\t%s\t%s' % (itr, llh, itr_time) for itr, llh, itr_time in llhs_and_times])
    ...
    savetxt(state['clonal_freqs_file'], vstack((glist, array([freq[g] for g in freq.keys()]).T)), fmt='%s',
            delimiter=', ')
...
def run(safe_to_exit, run_succeeded, config):
    ...
    if 0:#state_manager.state_exists():
        print 0
